...
- <a href="#sec-lifecycle" id="toc-sec-lifecycle"><span
  class="toc-section-number">1</span> Lifecycle and folder structure</a>
- <a href="#sec-json" id="toc-sec-json"><span
  class="toc-section-number">2</span> JSON format</a>
- <a href="#sec-items" id="toc-sec-items"><span
  class="toc-section-number">3</span> Schema fields</a>
  - <a href="#sec-attr" id="toc-sec-attr"><span
    class="toc-section-number">3.1</span> General Attributes</a>
  - <a href="#sec-simple" id="toc-sec-simple"><span
    class="toc-section-number">3.2</span> Simple fields</a>
  - <a href="#sec-multiple" id="toc-sec-multiple"><span
    class="toc-section-number">3.3</span> Multiple-choice</a>
  - <a href="#sec-object" id="toc-sec-object"><span
    class="toc-section-number">3.4</span> Composite field</a>
- <a href="#sec-full" id="toc-sec-full"><span
  class="toc-section-number">4</span> Full example</a>

This article describes how the metadata schemas used in the ManGO portal
are stored, from the folder structure that supports the schemas
lifecycle to the JSON format that codes the different fields and their
characteristics.
...
In the ManGO infrastructure, schemas belong to “realms”, which can be
projects, home collections, the public collection… When designing a
schema, one must first select a realm; a schema designed within a
certain realm can only be used to apply metadata to data of that realm.
...
first. It can be edited, viewed and deleted, but it cannot be used to
apply metadata.

Once a draft is **published**, it cannot be edited or deleted anymore,
but it can be viewed and, more importantly, it can be used to apply
metadata. Attempts to edit the a published version of a schema will
result in the creation of a new draft with a higher version number. If
this draft is then published, the current published version is
**archived**.
...
cannot be viewed either, but eventually it will be possible to have an
“advanced view” in the metadata schema manager that makes archived
versions available for viewing. A published version can also be
purposefully archived, without having to publish a draft that replaces
it. You can still have data with metadata based on an archived version
of a schema, but if you try to reapply the metadata schema you will only
be able to use the current published version, overriding any differences
between the version originally used for annotation and the current
version.

[Table 1](#tbl-lifecycle) summarizes what can be done with a metadata
schema version depending on its stage.

<div id="tbl-lifecycle">
...
The name of a file corresponding to a version of a schema follows the
following convention: `{schema_name}-v{version}(-{status}).json`, with
`status` being one of “draft” or “published”.

For example, when we first create the “book” schema used for
illustration in this document, a file will be created called
“book-v1.0.0-draft.json”, which will be stored inside the “book”
subdirectory of the “schemas” directory. As shown in
...
[Section 1](#sec-lifecycle), and next to `version` constitutes the main
...
implementing schemas as a more user-friendly label. The `edited-by`
...
which provides a human-readable label for both the form used in the
application of the schema and when viewing the metadata related to said
schema in the ManGO portal.
...
required when assigning metadata from the schema. If this attribute is
missing, it is read as “false”.

default  
Simple fields and single-value multiple-choice fields, *if required*,
may also contain a `default` attribute providing a default value for the
field.
...
field in a modal, except for simple fields, in which there is an
additional dropdown to select among its various subtypes.
...
the fields it is composed of, following the same criteria.
