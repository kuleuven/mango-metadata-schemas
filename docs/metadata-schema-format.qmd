...
:    Any object that represents a scalar field or an object field may contain an optional boolean `repeatable` attribute. If "true", the field may be used multiple times to assign the same attribute name with different values as metadata of the same collection or data object. If this attribute is missing, it is read as "false".
