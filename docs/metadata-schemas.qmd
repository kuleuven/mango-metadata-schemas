...
![Choose a realm.](used-screenshots/001-schema-start.png){#fig-start fig-alt="Accordion with a list of realms, one of which is enframed in a blue square with a cursor icon on top. Underneath and connected via an arrow, an empty metadata schema manager that just has the title and a button that reads 'new schema'."}

![An empty schema to start with.](used-screenshots/03-empty-schema.png){#fig-schemaeditor fig-alt="Initial form for an empty schema under the 'New schema' button, with fields to insert the schema ID and label, a button to add a new element and buttons to submit the form."}
...
![Options to select a type of input field.](used-screenshots/002-fields-long.png){#fig-fields fig-alt="Four cards with examples of the different kinds of fields and buttons that open editors to instantiate each field."}
...
![Design a simple field.](used-screenshots/06-add-simple-field.png){#fig-simple fig-alt="Form to create a simple field."}

![Design a single-value multiple-choice field.](used-screenshots/10-add-single-value-multiple.png){#fig-radio fig-alt="Form to create a single-value multiple-choice field."}

![Design a multiple-value multiple-choice field.](used-screenshots/17-add-multiple-value-multiple.png){#fig-checkbox fig-alt="Form to create a multiple-value multiple-choice field."}

![Design a single-value multiple-choice field.](used-screenshots/24-add-composite-field.png){#fig-composite fig-alt="Form to create a composite field, which looks like an empty schema."}
...
![A filled form for a simple field.](used-screenshots/08-title-simple-field.png){#fig-simplefull fig-alt="Filled form to create a new simple field."}

![View of a designed simple field.](used-screenshots/09-after-adding-field1.png){#fig-simpleview fig-alt="View of an editing form for a schema to which the book title field has been added."}
...
![A filled form for a single-value multiple-choice field.](used-screenshots/15-publisher-svmc-field.png){#fig-radiofull fig-alt="Filled form to design a single-value multiple-choice field."}

![View of a designed single-value multiple-choice field.](used-screenshots/16-after-adding-field2.png){#fig-radioview fig-alt="View of an editing form for a schema to which a field with a dropdown has been added."}
...
![New draft.](used-screenshots/22-save-draft1.png){#fig-saved fig-alt="View of the draft of a schema as only tab in the accordion item of that schema."}
...
![A filled form for a composite field.](used-screenshots/26-author-composite-field.png){#fig-compositefull fig-alt="Filled form to edit a composite field, which looks like an editor for a schema, after adding three subfields."}

![View of a designed composite field.](used-screenshots/27-view-composite.png){#fig-compositeview fig-alt="View of the box corresponding to a composite field after adding it to a draft schema."}
...
![A published version of a schema.](used-screenshots/34-view-published.png){#fig-published fig-alt="View of a published version of a schema as only tab in the accordion item of a schema."}
...
![A published version of a schema when a draft exists.](used-screenshots/38-view-published2.png){#fig-published2 fig-alt="View of a published version of a schema as first tab in the accordion item, with a draft as the second tab."}

![Draft a new schema from a published version of a schema.](used-screenshots/39-clone-published.png){#fig-clone fig-alt="Editor of a schema to create a copy or clone from a published schema."}

Create new drafts from a published version.
:::

When this copy is saved, a new schema is created, like when we edit one from scratch in "New schema". This generates a new accordion item with its own "draft" tab containing the version that was just created. Note that it is also possible to publish a version of a schema, even a copy from a published schema, without saving it as a draft first. Inf @fig-clone2 we could decide to view and edit this new schema or the previous one by clicking on their name, which expands the appropriate tabs. If we go to "Book schema as an example", we'll see that the "Copy to new schema" section has been reset to the original contents of the published version of this schema.

![Saved draft of copied schema.](used-screenshots/40-save-clone.png){#fig-clone2 fig-alt="View of the draft tab in the accordion of the new schema copied from a published schema, under the closed accordion item of the previous schema."}
